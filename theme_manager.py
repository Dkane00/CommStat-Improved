...
def prepare_platform_env_defaults() -> None:
    """Set environment variables needed for system theme integration.

    Must be called BEFORE QApplication is created so Qt picks up the
    platform theme plugin (e.g. qt5ct) on Linux.

    On macOS and Windows this is a no-op — those platforms use the
    hardcoded legacy colors in ThemeManager._LEGACY_COLORS instead.
    """
    import os
